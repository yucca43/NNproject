...
class RNN:
    ...
    def forwardProp(self,node,correct=[], guess=[]):
        ...
        # Recursion, if true, L&R both true?
        # if this is leaf!
        if(node.isLeaf):
            node.hActs1 = self.L[:,node.word]
        else:
            # some tree node might only have left node. [Directly copy from left?]
    ...
    def backProp(self,node,error=None):
        # skip this node if this node has only left child
